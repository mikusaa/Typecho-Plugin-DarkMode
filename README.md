...
- 在插件设置页检查并安装 GitHub 仓库中的最新版本

## 兼容性

- Typecho 1.3.0 及以上
- PHP 8.2 及以上
- PHP cURL 扩展（仅在线更新功能需要）
...
启用后，登录后的后台导航会显示当前外观的图标按钮；点击后会依次切换日间、夜间和跟随设备模式。登录和注册页面仍会应用当前外观偏好，但不显示切换按钮。
...
进入“控制台 > 插件”，打开 DarkMode 的“设置”页面并点击“检查并更新插件”。插件会从本仓库的 `main` 分支下载文件清单，校验每个文件的 SHA-256 后再替换本地文件。

在线更新需要 PHP cURL 扩展以及插件目录的写入权限。更新完成后，请停用并重新启用 DarkMode。若任一文件下载、校验或替换失败，更新器会停止操作，并回滚本次已经替换的文件。
